...
int main(int argc, char *argv[]) {
	...
	contcarac(cad,v);
	reporte(v);
	cout<<"la cadena tiene " <<cuentapalabras(cad)<<" palabras."<<endl;
	return 0;
}
...
void reporte(int valfab[27]){
	int i;
	cout<<" a) Numero de veces que sucede cada letra. "<<endl;
...
int cuentapalabras(char cad[100]){
	int x=0,cont=0,n;
	//char aux[100][50];
	//for(i=0;i<=10;i++)
		//v[i]=0;
	...
	while(x<n)
	{
		if(cad[x]==' ')
			b=false;
		if(((cad[x]>96 && cad[x]<123)||(cad[x]>64 && cad[x]<91) || cad[x]=='ñ' || cad[x]=='Ñ') && b==false ) //cad[x]!=' '
		{
			cont++;
			b=true;
